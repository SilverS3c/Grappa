...
class TestGrappa(unittest.TestCase):
    ...
    def testQuery(self):
        request = {"targets": [{"target": "ram_usage", "payload": {}}], "range": {"from": str((datetime.datetime.fromtimestamp(1699266108) - datetime.timedelta(hours=2)).isoformat()) + ".0Z", "to": str((datetime.datetime.fromtimestamp(1699266108) + datetime.timedelta(hours=2)).isoformat()) + ".0Z"}}
